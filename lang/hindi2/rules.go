// Package hindi2 provides Hindi-specific transliteration rules using the brahmic script layer.
package hindi2

import "github.com/budhash/gomanize/script/brahmic"

// Rules returns the Hindi-specific transliteration rules.
func Rules() []brahmic.Rule {
	return []brahmic.Rule{
		// === PhaseSchwa Rules ===
		...
		// Word-final conjuncts are handled separately by schwa-delete-word-final
		{
			...
			Phase:    brahmic.PhaseSchwa,
			Scope:    brahmic.ScopeScript,
			Priority: 80,
			Mode:     brahmic.ModeExclusive,
			Condition: func(u *brahmic.Unit, w *brahmic.Word) bool {
				if !isConsonantOrConjunct(u) || u.Schwa != brahmic.SchwaPending {
					return false
				}
			...
			Action: func(u *brahmic.Unit, w *brahmic.Word) {
				u.Schwa = brahmic.SchwaKeep
			},
		},
		...
		// Also applies for C+C+Modifier (anusvara/chandrabindu): झारखंड→jharkhand
		{
			...
			Condition: func(u *brahmic.Unit, w *brahmic.Word) bool {
				...
				if afterNext.Type != brahmic.UnitVowel && afterNext.Type != brahmic.UnitModifier {
					return false
				}
				return true
			},
			Action: func(u *brahmic.Unit, w *brahmic.Word) {
				u.Schwa = brahmic.SchwaDelete
				if u.Run != nil {
		...
		// Default: keep schwa if no other rule decided
		{
			...
			Scope:    brahmic.ScopeUniversal,
			Priority: 0,
			Mode:     brahmic.ModeFallback,
			Condition: func(u *brahmic.Unit, w *brahmic.Word) bool {
				return isConsonantOrConjunct(u) && u.Schwa == brahmic.SchwaPending
			},
			Action: func(u *brahmic.Unit, w *brahmic.Word) {
				u.Schwa = brahmic.SchwaKeep
			},
		...
		// व→w after स, श, द, ख: स्वागत→swagat, ऐश्वर्या→aishwarya
		{
			...
			Phase:    brahmic.PhaseConsonant,
			Scope:    brahmic.ScopeLanguage,
			Priority: 50,
			Mode:     brahmic.ModeAlways,
			Condition: func(u *brahmic.Unit, w *brahmic.Word) bool {
		...
		{
			Name:     "long-aa-all",
			Phase:    brahmic.PhaseVowel,
			Scope:    brahmic.ScopeScheme,
			Priority: 60,
			Mode:     brahmic.ModeAlways,
			Condition: func(u *brahmic.Unit, w *brahmic.Word) bool {
...
func isConsonantOrConjunct(u *brahmic.Unit) bool {
	return u.Type == brahmic.UnitConsonant || u.Type == brahmic.UnitConjunct
}
