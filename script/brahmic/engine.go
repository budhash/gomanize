...
//  2. Rules: Apply schwa deletion and other transformation rules
...
type Language interface {
	...
	// Nukta returns the nukta character for this script (empty if none).
	Nukta() string
}
...
// Engine is the main romanization engine for Brahmic scripts.
type Engine struct {
	language   Language
	parser     *Parser
	renderer   *Renderer
...
func New(lang Language) *Engine {
	e := &Engine{
		language:   lang,
		parser:     NewParser(lang.Symbols(), lang.MultiChar(), lang.Halant(), lang.Nukta()),
...
// Transliterate converts Brahmic script text to romanized form using default options.
func (e *Engine) Transliterate(input string) string {
	return e.TransliterateWithOptions(input, DefaultOptions())
}

// TransliterateWithOptions converts Brahmic script text to romanized form with custom options.
